...
foreach($server as $serv) {
	...
	$vhost_update= "INSERT INTO wow_vhost_status (session_time, vhost_name, vhost_timerunning, vhost_conn_limit, vhost_conn_current, vhost_conn_total, vhost_conn_accepted, vhost_conn_rejected, 
		vhost_bytes_in, vhost_bytes_out) 
		VALUES (CURRENT_TIMESTAMP, '$VHostName', '$VHostTimeRunning', '$VHostConnectionsLimit', '$VHostConnectionsCurrent', '$VHostConnectionsTotal', '$VHostConnectionsTotalAccepted', 
			'$VHostConnectionsTotalRejected', '$VHostMessagesInBytesRate', '$VHostMessagesOutBytesRate ')";
	if (mysqli_query($connect, $vhost_update)) {
		//only enable for debugging
		echo " | Added new Data: <img src='". $DOCUMENT_ROOT . "/img/online-16.png'>";
	}	
	else {
...
if (mysqli_query($connect, $clean1)) {
	//only enable for debugging
	echo  " | Table wow_app_status: <img src='". $DOCUMENT_ROOT . "/img/online-16.png'>";
}	
else {
	echo "Error: " . $clean1. "<br>" . mysqli_error($connect);
}	
...
foreach($application as $app) {
	...
	if (mysqli_query($connect, $app_update)) {
		echo " | Added new App Data <img src='". $DOCUMENT_ROOT . "/img/online-16.png'>";
	}	
	else {
